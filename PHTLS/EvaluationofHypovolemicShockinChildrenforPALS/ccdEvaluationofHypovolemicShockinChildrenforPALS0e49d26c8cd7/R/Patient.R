...
#' \code{Patient.}
#'
#' Returns a data.frame of the collected nodes of: \code{Patient} from the XML instance passed as fileName.
#' The source name and the CCD ID are also included in each row to assist in identifying the source of the data.
#' The XML element name is pcs-487b0c6a-fd6b-4ab6-9de8-c262ab3a28f5 as a restriction of the DvString
...
#' Patient <- getPatient(files) 
#' 
#' @export
getPatient <- function(sourceList)
{
    data <- lapply(sourceList, parsePatient)
    data <- data.table::rbindlist(data, fill=TRUE)
    return(data)
}

#' @export
parsePatient <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-487b0c6a-fd6b-4ab6-9de8-c262ab3a28f5', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvString)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-487b0c6a-fd6b-4ab6-9de8-c262ab3a28f5'
      data$ccd <- XML::xmlName(root)
