...
#' \code{Gender of a Person.}
#'
#' Returns a data.frame of the collected nodes of: \code{Gender of a Person} from the XML instance passed as fileName.
#' The source name and the CCD ID are also included in each row to assist in identifying the source of the data.
#' The XML element name is pcs-c0090ba9-4408-48a7-899a-346f59d40ff7 as a restriction of the DvString
...
#' GenderofaPerson <- getGenderofaPerson(files) 
#' 
#' @export
getGenderofaPerson <- function(sourceList)
{
    data <- lapply(sourceList, parseGenderofaPerson)
    data <- data.table::rbindlist(data, fill=TRUE)
    return(data)
}

#' @export
parseGenderofaPerson <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-c0090ba9-4408-48a7-899a-346f59d40ff7', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvString)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-c0090ba9-4408-48a7-899a-346f59d40ff7'
      data$ccd <- XML::xmlName(root)
